...
pub enum LetLangExp {  // set of possible LetLangExp's
    ...
    ProcExp(VarExp, Rc<LetLangExp>, Rc<let_lang_env::LetLangEnv>),  //proc-exp(var, body)
    CallExp(Rc<LetLangExp> ,Rc<LetLangExp>), //call-ex(rator, rand)
}
...
impl LetLangExp {
    ...
    pub fn new_proc_exp(v: &String, arg: &LetLangExp, env: &let_lang_env::LetLangEnv) -> Self{
        LetLangExp::ProcExp(v.clone(), Rc::new(arg.clone()), Rc::new(env.clone()));
    }
    pub fn new_call_exp(arg1: &LetLangExp, arg2: &LetLangExp) -> Self {
        LetLangExp::CallExp(Rc::new(arg1.clone()), Rc::new(arg2.clone()))
    }
    ...
    pub fn to_string(&self) -> String {
        match self.clone() {
            ...
            LetLangExp::LetExp(v, e1, e2)   => {let mut temp = "let ".to_string();
                                                ...
                                                temp.push_str(&(e2.to_string()));
                                                temp}
            LetLangExp::ProcExp(v, e)       => {let mut temp= "proc( ".to_string();
                                                temp.push_str(&(v.to_string()));
                                                temp.push_str(&(") ".to_string()));
                                                temp.push_str(&(e.to_string()));
                                                temp}
            LetLangExp::CallExp(e1, e2)     => {let mut temp= "(".to_string();
                                                temp.push_str(e1.to_string());
                                                temp.push_str(" ");
                                                temp.push_str(e2.to_string());
                                                temp.push_str(")");
